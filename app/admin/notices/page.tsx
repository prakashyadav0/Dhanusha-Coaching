...
export default function AdminNoticesPage() {
  ...
  async function submitNotice(e: React.FormEvent) {
    ...
    setSaving(false);
    if (res.ok) { setMsg('Notice posted!'); setShowModal(false); setForm({ title: '', body: '', targetRole: 'all', isPinned: false }); fetchNotices(); }
              ...
              <div className="flex items-start justify-between">
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    {n.isPinned && <span className="text-xs bg-amber-100 text-amber-700 px-2 py-0.5 rounded-full font-medium">📌 Pinned</span>}
                    <span className="text-xs bg-gray-100 text-gray-600 px-2 py-0.5 rounded-full capitalize">{n.targetRole}</span>
                  </div>
                  <h3 className="font-semibold text-gray-900">{n.title}</h3>
                  <p className="text-sm text-gray-600 mt-1">{n.body}</p>
                  <p className="text-xs text-gray-400 mt-2">By {n.postedBy?.name} · {new Date(n.createdAt).toLocaleDateString()}</p>
                </div>
                <button onClick={() => deleteNotice(n._id)} className="text-xs text-red-500 hover:text-red-700 ml-4 shrink-0">Delete</button>
              </div>
            </div>
          ))}
        </div>
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg p-6">
            ...
            <form onSubmit={submitNotice} className="space-y-4">
              ...
              <div>
                ...
                <textarea required value={form.body} onChange={e => setForm({...form, body: e.target.value})} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 h-28 resize-none" placeholder="Notice content..." />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Target</label>
                <select value={form.targetRole} onChange={e => setForm({...form, targetRole: e.target.value})} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">
              ...
              <div className="flex justify-end gap-3 pt-2">
                ...
                <button type="submit" disabled={saving} className="px-4 py-2 text-sm text-white bg-indigo-600 rounded-lg hover:bg-indigo-700 disabled:opacity-60 transition">{saving ? 'Posting...' : 'Post Notice'}</button>
              </div>
            </form>
          </div>
        </div>
